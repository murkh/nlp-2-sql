...
class SelfConsistencyManager:
    ...
    def generate_and_select(
        ...
        logger.info(
            f"Generating {num_candidates} candidate SQL queries for self-consistency..."
        )
        ...
        for i, temp in enumerate(temperatures):
            ...
            if not generated_json or not generated_json.get(app_consts.SQL):
                logger.warning(
                    f"Candidate {i} generation returned invalid JSON structure."
                )
                continue
        ...
        if len(candidates) == 1:
            logger.info(
                "Only one valid candidate query generated, selecting it directly."
            )
            return candidates[0]
        ...
        if not selector_output or "selected_index" not in selector_output:
            logger.warning(
                "Selector failed or returned invalid response, defaulting to candidate 0."
            )
            return candidates[0]
        ...
        if 0 <= selected_idx < len(candidates):
            ...
            logger.warning(
                f"Selector returned out-of-bounds index {selected_idx}, defaulting to candidate 0."
            )
            return candidates[0]
